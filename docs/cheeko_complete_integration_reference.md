# Cheeko Complete Integration Reference

**Audience:** Firmware engineers, backend engineers, or anyone who needs to understand the full runtime flow between the ESP32 device and all Cheeko backend services.

**Covers:** Every HTTP API call, every MQTT message (both directions), every UDP packet contract, all state machine transitions, all RFID/content flows, and what happens internally on the gateway side.
...
10. [Phase 8 – Session End (Goodbye)](#10-phase-8--session-end-goodbye)
11. [RFID Flow (All Cases)](#11-rfid-flow-all-cases)
12. [Device State Machine](#12-device-state-machine)
13. [All MQTT Message JSON Structures](#13-all-mqtt-message-json-structures)
14. [UDP Packet Format](#14-udp-packet-format)
15. [All Manager API Endpoints Used](#15-all-manager-api-endpoints-used)
16. [Gateway Internal API Calls (to Manager API)](#16-gateway-internal-api-calls-to-manager-api)
17. [LiveKit Data Channel Messages (Gateway ↔ Agent)](#17-livekit-data-channel-messages-gateway--agent)
...
All device→server communication starts with the Manager API (OTA), then shifts to the MQTT Gateway for real-time protocol.
...
**Body (Spring Boot DeviceReportReqDTO format):**
```json
{
...
- If `firmware.force = 1`, must update immediately before connecting.
...
|---|---|---|
| `200` | `success` (plain text) | Device is activated, proceed |
| `202` | (empty) | Not activated yet, retry after ~3s |
| `202` | (empty) | Any error also returns 202 |

**Firmware rules:**
- Loop polling until `200 success`.
- Current behavior: up to 10 retries per cycle, ~3s between each.
- Display activation code (`activation.code` from OTA response) to user during loop.
...
From the OTA response `mqtt` object:
...
- `transport: "udp"` — firmware requests UDP audio channel
...
When gateway receives hello from device:
...
5. Immediately sends server hello (below).
...
**Field notes:**
- `audio_params.sample_rate: 24000` — server sends 24kHz downlink audio (different from device uplink)
...
**Timeout:** Firmware should wait max 10s for server hello; if not received, retry or fall back.

---

## 6. Phase 4 – UDP Channel Setup

After receiving server hello, firmware:

1. Opens UDP socket to `udp.server` : `udp.port`.
2. Configures AES-128-CTR cipher with `udp.key` and `udp.nonce`.
3. Starts listening for downlink audio from gateway.

See [Section 14](#14-udp-packet-format) for the exact packet structure.

**Uplink (firmware → gateway):** Mic audio (16kHz Opus, encrypted)
**Downlink (gateway → firmware):** Assistant TTS audio (24kHz Opus, encrypted)

---

## 7. Phase 5 – Mode Update (Deferred)

After hello, gateway queries Manager API in background:
...
Then gateway sends `mode_update` to device:
...
After mode_update, gateway dispatches the LiveKit agent:
- `CHARACTER_AGENT_MAP` resolves character to agent worker:
  - `Cheeko` → `cheeko-agent`
  - `Math Tutor` → `math-tutor-agent`
  - `Riddle Solver` → `riddle-solver-agent`
  - `Word Ladder` → `word-ladder-agent`

**Note:** Current firmware does not consume `mode_update` in its incoming message handler. This is a known integration gap (see [Section 18](#18-what-is-missing-from-existing-firmware-guide)).
...
Used to log/surface wake word detection event. Separate from `listen start`.
...
- Used for facial animation / LED / display updates.
...
Sent when LiveKit agent has joined the room and is ready to process audio. Firmware can use this to know the AI is ready.
...
**Other `reason` values:** `button_pressed`, `user_interrupt`

**Firmware behavior on sending abort:**
1. Clear pending UDP uplink buffers.
2. Immediately start new `listen start`.
3. Do NOT full reconnect — reuse existing session/UDP channel.

**Gateway behavior on receiving abort:**
...
## 10. Phase 8 – Session End (Goodbye)
...
**Other `reason` values:** `error`, `disconnect`, `mode_change`

**Firmware behavior on receiving goodbye:**
1. Close UDP channel.
2. Reset session variables (`session_id`, crypto state).
3. Transition to `idle`.
4. Do NOT MQTT disconnect — stay connected for next session.

---

## 11. RFID Flow (All Cases)
...
2. Returns one of three responses below.
...
```json
{
  ...
  "version": 1,
  "audio": [
    {"index": 1, "url": "https://.../audio/track1.mp3"},
    {"index": 2, "url": "https://.../audio/track2.mp3"}
  ],
  "images": [
    {"index": 1, "url": "https://.../images/page1.jpg"},
    {"index": 2, "url": "https://.../images/page2.jpg"}
  ]
}
```
...
- Write `manifest.jsn` **last** — it is the completion marker. If firmware crashes mid-download, next boot will retry download.
...
The Manager API `card/lookup` response `contentType` field:
...
For `prompt` and `read_only` cards, firmware may receive a `tts start` / audio response without having sent a `listen start` — this is the gateway routing a scripted response.

### 11.7 Legacy Download Response (Gateway Also Supports)

Gateway also handles older `download_response` format (for backward compatibility):
...
## 13. All MQTT Message JSON Structures
...
**Note:** Firmware records at 16kHz; gateway resamples uplink from 16kHz to 24kHz before sending to LiveKit. Assistant audio comes back at 24kHz and is sent as-is to the firmware for playback. Firmware DAC must support 24kHz playback.

### 14.5 Validation Rules

1. Ignore packets where `packet_type != 1`.
2. Ignore packets where header length < 16.
3. Track remote `sequence` number; reject stale out-of-order packets.
4. Keep local `sequence` counter for uplink; increment per packet.

---

## 15. All Manager API Endpoints Used
...
|---|---|---|---|
| `POST` | `/ota/` | None (public) | Boot OTA check, get MQTT creds |
...
| `POST` | `/config/child-profile-by-mac` | Get child profile (age, name, prefs) |
...
**Exception:** `/toy/ota/` returns raw JSON (no wrapper).
**Exception:** `/toy/ota/activate` returns plain text `success` or empty body.

---

## 16. Gateway Internal API Calls (to Manager API)
...
After these complete, gateway:
1. Sends `mode_update` to device.
2. Dispatches appropriate LiveKit agent.
...
## 17. LiveKit Data Channel Messages (Gateway ↔ Agent)

These are internal messages between the MQTT Gateway and the LiveKit Python agent workers. Firmware does not see these directly — they drive the AI behavior.

### 17.1 Gateway → Agent (sent to LiveKit data channel)
...
### 17.2 Agent → Gateway (events consumed by LiveKitBridge)
...
### 17.3 cheeko_worker.py Handled `data_received` Types
...
**Known gap:** `ptt_event`, `speech_end`, `abort` are sent by gateway to agent but the worker does not have explicit handlers for these — they are handled by the LiveKit SDK's built-in agent session flow.

---

## Appendix: MQTT Topic Summary
...
| EMQX → Gateway | `internal/server-ingest` | Internal EMQX republish; envelope has `sender_client_id` + `orginal_payload` (typo intentional) |

## Appendix: Session ID Format
...
| RFID API call (gateway) | 5s | Timeout on Manager API call |
