# Child Game Achievements Dashboard — Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a "Game Progress & Achievements" section to the KidProfiles page in manager-web, showing per-child game levels, streaks, achievements, and session history fetched from the game progression API.

**Architecture:** New Vue component `KidGameProgress.vue` rendered as an expandable section within the existing `KidProfiles.vue`. API calls go through a new `game.js` API module hitting the existing `/toy/game/*` endpoints (service key auth via admin proxy).

**Tech Stack:** Vue 2.6, Element UI 2.15, SCSS, FlyIO HTTP client
...
|------|--------|---------------|
| `src/views/KidGameProgress.vue` | Create | Game progress section component |
| `src/apis/module/game.js` | Create | Game progression API calls |
| `src/apis/api.js` | Modify | Register game API module |
| `src/views/KidProfiles.vue` | Modify | Add expandable game progress section per kid |
| `src/routes/game.routes.js` (manager-api-node) | Modify | Add admin-proxied endpoints |

---

## Task 1: Create game API module

**Files:**
- Create: `main/manager-web/src/apis/module/game.js`
- Modify: `main/manager-web/src/apis/api.js`

- [ ] **Step 1: Create game.js API module**

```javascript
/**
 * Game progression API endpoints.
 * Uses admin endpoints that proxy to service-key-protected game routes.
 */
import RequestService from '../httpRequest'

export default {
  // GET /admin/game/progress/:kidId
  getProgress(kidId, callback) {
    RequestService.sendRequest()
      .url(`/admin/game/progress/${kidId}`)
      .method('GET')
      .success(callback)
      .send()
  },

  // GET /admin/game/streak/:kidId
  getStreak(kidId, callback) {
    RequestService.sendRequest()
      .url(`/admin/game/streak/${kidId}`)
      .method('GET')
      .success(callback)
      .send()
  },

  // GET /admin/game/achievements/:kidId
  getAchievements(kidId, callback) {
    RequestService.sendRequest()
      .url(`/admin/game/achievements/${kidId}`)
      .method('GET')
      .success(callback)
      .send()
  },

  // GET /admin/game/sessions/:kidId
  getSessions(kidId, callback) {
    RequestService.sendRequest()
      .url(`/admin/game/sessions/${kidId}`)
      .method('GET')
      .success(callback)
      .send()
  }
...
Add import and export in `src/apis/api.js`:
...
```bash
git add main/manager-web/src/apis/module/game.js main/manager-web/src/apis/api.js
git commit -m "feat(dashboard): add game progression API module"
```

---

## Task 2: Add admin proxy routes in manager-api-node

**Files:**
- Modify: `main/manager-api-node/src/routes/game.routes.js`

The manager-web uses Bearer token auth (admin), but game routes use service key auth. Add admin-proxied endpoints that internally call the service-key routes.

- [ ] **Step 1: Add admin endpoints to game.routes.js**

Add these routes BEFORE the existing service-key routes:
...
router.get('/admin/sessions/:kidId', requireAdmin, asyncHandler(async (req, res) => {
...
- [ ] **Step 2: Update game.js API to use admin paths**

```javascript
// GET /game/admin/progress/:kidId (not /admin/game/)
getProgress(kidId, callback) {
  RequestService.sendRequest()
    .url(`/game/admin/progress/${kidId}`)
    .method('GET')
    .success(callback)
    .send()
}
...
```bash
git add main/manager-api-node/src/routes/game.routes.js main/manager-web/src/apis/module/game.js
git commit -m "feat(dashboard): add admin-proxied game progress endpoints"
```

---

## Task 3: Create KidGameProgress component

**Files:**
- Create: `main/manager-web/src/views/KidGameProgress.vue`
  ...
  <div class="kid-game-progress" v-loading="loading">
    ...
    <div class="sessions-section" v-if="sessions.length > 0">
      ...
      <el-table :data="sessions" size="small" stripe>
        <el-table-column prop="game_type" label="Game" width="120">
          <template slot-scope="{ row }">{{ gameName(row.game_type) }}</template>
        </el-table-column>
  ...
  computed: {
    ...
    upcomingAchievements() {
      ...
      const ALL_UPCOMING = [
        ...
        { code: 'perfect_round', label: 'Perfect Round', hint: 'Get 5/5 correct' },
        { code: 'no_hints', label: 'No Hints', hint: 'Complete a round without hints' },
      ...
      return ALL_UPCOMING.filter(a => !unlocked.has(a.code)).slice(0, 4)
    },
    levelHistory() {
      // Group sessions by game_type + level, count attempts per level
      const groups = {}
...
```bash
git add main/manager-web/src/views/KidGameProgress.vue
git commit -m "feat(dashboard): create KidGameProgress component"
```

---

## Task 4: Wire into KidProfiles page

**Files:**
- Modify: `main/manager-web/src/views/KidProfiles.vue`
...
Replace the `<el-table>` with expandable rows:

```vue
<el-table :data="kidProfiles" v-loading="loading" @expand-change="onExpandRow">
  <el-table-column type="expand">
    <template slot-scope="{ row }">
      <kid-game-progress :kid-id="row.name" />
    </template>
  </el-table-column>
  <!-- ...existing columns -->
</el-table>
...
Note: `kid-id` uses `row.name` because the game engines currently use the child's name as the `childId` in progression calls. This should eventually be changed to `row.id`.

- [ ] **Step 3: Commit**

```bash
git add main/manager-web/src/views/KidProfiles.vue
git commit -m "feat(dashboard): add expandable game progress to kid profiles table"
```
...
| Task | Files | What |
|------|-------|------|
| 1 | game.js, api.js | API module for game progression |
| 2 | game.routes.js, game.js | Admin proxy endpoints (Bearer auth) |
| 3 | KidGameProgress.vue | Stats cards, achievements, sessions table |
| 4 | KidProfiles.vue | Wire component into existing page |
