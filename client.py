...
class TestClient:
    ...
    def trigger_conversation(self):
        ...
        try:
            ...
            timeout_count = 0
            logger.info(f"[LOOP] Entering main loop. stop_threads={stop_threads.is_set()}, session_active={self.session_active}")
            while not stop_threads.is_set() and self.session_active:
                time.sleep(1)
            ...
            logger.info(f"[LOOP] Exited main loop. stop_threads={stop_threads.is_set()}, session_active={self.session_active}")

        except KeyboardInterrupt:
            logger.info("Manual interruption detected (KeyboardInterrupt). Cleaning up...")
            stop_threads.set()
            self.session_active = False
        except Exception as e:
            logger.error(f"[ERROR] Unexpected exception in main loop: {e}")
        return True
    ...
    def run_test(self):
        ...
        if not self.get_ota_config():
            logger.error("[EXIT] Exiting due to OTA config failure")
            return
        if not self.connect_mqtt():
            logger.error("[EXIT] Exiting due to MQTT connection failure")
            return
        time.sleep(1)  # Give MQTT a moment to connect and subscribe
        if not self.send_hello_and_get_session():
            logger.error("[EXIT] Exiting due to hello/session failure")
            self.cleanup()
            return
        logger.info("[RUN] Starting trigger_conversation - press 's' for greeting, 'p' for PTT")
        self.trigger_conversation()
        logger.info("[EXIT] trigger_conversation returned, cleaning up")
        self.cleanup()
