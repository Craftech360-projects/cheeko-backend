...
class TestClient:
    def __init__(self, device_mac: Optional[str] = None, board_type="cheeko"):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--board-type",
        choices=["cheeko", "ai_printer"],
        default=os.getenv("TEST_BOARD_TYPE", "cheeko"),
        help="Device board type: 'cheeko' (voice AI) or 'ai_printer' (line art).",
    )
    ...
    print(f"[MODE] Running as: {args.board_type}")
    ...
    client = TestClient(device_mac=args.device_mac, board_type=args.board_type)
    try:
        if args.mode == "voice":
            client.run_test()
        else:
