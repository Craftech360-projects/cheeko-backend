...
DAILY_TEN = 10  # manager DAILY_QUESTION_TARGET
...
def quiz_block(batch: dict | None) -> str:
    ...
    # STATUS and the rules below are picoclaw quiz_bank.go wording
    if batch.get("day_complete"):
        lines.append(f"STATUS: today's Daily Ten IS complete ({answered} scored today). Do not ask another scored question "
                     f"today; celebrate and offer one unscored Bonus Buzz.")
    else:
        lines.append(f"STATUS: today's Daily Ten is NOT complete - {answered} of {DAILY_TEN} scored so far today, and the "
                     f"questions below are the ones still to ask. Ignore anything in the conversation or your memory that "
                     f"says today is finished; this line is computed from the record and overrides it.")
    lines.append("Ask ONLY these questions, in order, one per turn. Never invent a question.")
    lines.append("If the child does not answer, encourage and re-ask the same question — never move on to the next question "
                 "until this one has been judged.")
    if batch.get("replay"):
        lines.append("These are champion rounds — the child has beaten every level; frame them as a victory lap.")
    # numbered from answered+1, not 1: a list restarting at 1 made the model say "question one" mid-day
    for i, q in enumerate(batch["questions"], answered + 1):
