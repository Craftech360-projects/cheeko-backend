...
class FunASRSTT(stt.STT):
    ...
    def _audio_buffer_to_bytes(self, buffer: utils.AudioBuffer) -> bytes:
            ...
            try:
                ...
                for frame in buffer:
                    if hasattr(frame, 'data'):
                        ...
                        # RESAMPLING DISABLED: RoomInputOptions handles sample rate conversion
        ...
        # RESAMPLING DISABLED: RoomInputOptions already converts audio to 16kHz
        # Log if there's a mismatch (for debugging only)
        if source_sample_rate and source_sample_rate != target_sample_rate:
            logger.debug(f"Sample rate mismatch detected: buffer={source_sample_rate}Hz, expected={target_sample_rate}Hz (RoomInputOptions should handle this)")
        # if source_sample_rate and source_sample_rate != target_sample_rate:
        ...
        try:
            async for frame in self._input_ch:
                ...
                if isinstance(frame, rtc.AudioFrame):
                    ...
                    # RESAMPLING DISABLED: RoomInputOptions handles sample rate conversion to 16kHz
