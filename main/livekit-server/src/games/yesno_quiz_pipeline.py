"""
Pipeline configuration for Yes/No Quiz game.
...
def create_pipeline(yaml_config: dict = None):
    ...
    try:
        stt = assemblyai.STT(
            ...
            keyterms_prompt=["yes", "no", "yeah", "nah", "haan", "nahi", "true", "false"],
        )
    ...
    llm_base_url = os.getenv("YESNO_LLM_BASE_URL", "")
    llm_temperature = float(os.getenv("YESNO_LLM_TEMPERATURE", "0.6"))
    ...
    # --- TTS: Deepgram Aura-2 ---
    dg_voice = os.getenv("YESNO_DEEPGRAM_TTS_VOICE", "athena")
    try:
        tts = inference.TTS(
