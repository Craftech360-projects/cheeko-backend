...
SERVER_IP = "manager-api-production-e079.up.railway.app"
OTA_PORT = 8002
# Use Railway's TCP proxy endpoint for external MQTT connections
MQTT_BROKER_HOST = "nozomi.proxy.rlwy.net"

MQTT_BROKER_PORT = 52879  # Railway's proxied port
...
class TestClient:
    ...
    def get_ota_config(self) -> bool:
        ...
        logger.info(
            f"[STEP] STEP 1: Requesting OTA config from https://{SERVER_IP}/toy/ota/")
        try:
            ...
            response = requests.post(
                f"https://{SERVER_IP}/toy/ota/", headers=headers, json=data, timeout=5)
                    ...
                    for attempt in range(10):
                        ...
                        try:
                            status_response = requests.get(
                                f"https://{SERVER_IP}/ota/active", params={"mac": self.device_mac_formatted}, timeout=3)
                            print(
