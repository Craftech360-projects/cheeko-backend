...
async def entrypoint(ctx: JobContext):
    ...
    # Use Gemini's default VAD (matches working browser reference)
    # Firmware controls audio streaming (only sends while button held)
    # No custom VAD config needed - let Gemini handle it naturally
    realtime_model = google.realtime.RealtimeModel(
        ...
        # No realtime_input_config - use Gemini's optimized defaults
    )

    logger.info(f"✅ Gemini Realtime model created with default VAD (matches working browser reference)")

    # Create AgentSession with default settings
