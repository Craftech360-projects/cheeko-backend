"""
Cheeko German Agent Worker
Conversational agent that speaks only in German
...
MEMORY_TRIGGER_PATTERNS = [
    ...
    ("german", ["auf deutsch", "sprich deutsch", "german", "deutsch"]),
]
...
class CheekoGermanAssistant(BaseAssistant):
    """Cheeko German Assistant - German-speaking companion"""

    GREETING_INSTRUCTION = (
        "Greet the child warmly as Cheeko, a friendly AI companion. "
        "You MUST speak ONLY in German. Keep it brief and playful. "
        "Say something like 'Hallo! Ich bin Cheeko, dein lustiger Freund!'"
    )
...
async def entrypoint(ctx: JobContext):
    """Entrypoint for Cheeko German agent worker"""
    ...
    async def inject_memory_context(user_query: str, mac_address: str, category: str):
        ...
        try:
            ...
            if relevant_memories:
                ...
                if category in ["story", "remember", "question", "german"]:
                    try:
    ...
    # Create assistant
    assistant = CheekoGermanAssistant(instructions=agent_prompt)
    ...
    def on_data_received(data_packet: rtc.DataPacket):
        try:
            ...
            if msg_type == 'end_prompt':
                prompt_text = message.get('prompt', "Die Zeit vergeht so schnell! Es war wunderbar mit dir zu sprechen. Auf Wiedersehen!")
                        ...
                        if content_type == 'read_only' and content_text:
                            ...
                            try:
                                ...
                                if audio_data and not error:
                                    ...
                                    await session.generate_reply(instructions=f"Read this aloud in a warm, friendly German voice: {content_text}")
...
if __name__ == "__main__":
    port = int(os.getenv("CHEEKO_GERMAN_PORT", DEFAULT_PORT))
