...
async def entrypoint(ctx: JobContext):
    ...
    if not quota_manager.is_unbound:
        game_session_allowed, game_session_reason = await quota_manager.start_game_session(
            agent_type="riddle_solver", session_id=room_name
        )
    ...
    REMINDER_MESSAGES = [
        ...
        "Say ONLY this to the child and then STOP completely. Do NOT call any tools, do NOT answer questions, do NOT continue the conversation: 'No rush! Would you like me to repeat the riddle?'",
        "Say ONLY this to the child and then STOP completely. Do NOT call any tools, do NOT answer questions, do NOT continue the conversation: 'Still thinking? That is okay! Riddles take time. Let me know when you are ready.'",
    ]
        ...
        try:
            ...
            try:
                if quota_manager.in_game_session:
                    game_status = "completed" if assistant.riddle_game_state.streak >= 5 else "abandoned"
                    await asyncio.wait_for(
