"""
Cheeko Magic Agent Worker
Conversational agent themed as a magical wizard/sorcerer companion
...
MEMORY_TRIGGER_PATTERNS = [
    ...
    ("magic", ["show me magic", "magic trick", "do a spell", "cast a spell", "abracadabra"]),
]
...
class CheekoMagicAssistant(BaseAssistant):
    """Cheeko Magic Assistant - Magical wizard companion"""

    GREETING_INSTRUCTION = (
        "Greet the child as Cheeko the Magical Wizard! "
        "You are a friendly, mystical sorcerer who speaks with wonder and excitement. "
        "Keep it brief, playful, and sprinkle in magical words like 'abracadabra' or 'alakazam'."
    )
...
async def entrypoint(ctx: JobContext):
    """Entrypoint for Cheeko Magic agent worker"""
    ...
    async def inject_memory_context(user_query: str, mac_address: str, category: str):
        ...
        try:
            ...
            if relevant_memories:
                ...
                if category in ["story", "remember", "question", "magic"]:
                    try:
    ...
    # Create assistant
    assistant = CheekoMagicAssistant(instructions=agent_prompt)
    ...
    def on_data_received(data_packet: rtc.DataPacket):
        try:
            ...
            if msg_type == 'end_prompt':
                prompt_text = message.get('prompt', "The magic show must end for now! It was enchanting talking with you. Goodbye!")
                        ...
                        if content_type == 'read_only' and content_text:
                            ...
                            try:
                                ...
                                if audio_data and not error:
                                    ...
                                    await session.generate_reply(instructions=f"Read this aloud in a magical, enchanting voice: {content_text}")
...
if __name__ == "__main__":
    port = int(os.getenv("CHEEKO_MAGIC_PORT", DEFAULT_PORT))
