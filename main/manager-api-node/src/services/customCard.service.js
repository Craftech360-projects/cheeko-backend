...
// Bounds the *upload*, not what is stored. Every recording is re-encoded to a
// 24 kHz mono MP3 before it reaches storage (see toDeviceAudio), which lands a
// 10 MB WAV at roughly 1 MB, so there is no separate ceiling on the result.
...
/**
 * Re-encode a validated recording into the MP3 the toy stores and plays.
 *
 * Separate from the upload for the same reason toDeviceImage is: this is the
 * last step that can fail cheaply, so a batch converts everything before it
 * stores anything and a bad third recording leaves the first two out of the
 * bucket.
 *
 * A conversion failure past validation means a file whose header is honest but
 * whose body is not — a truncated MP3, say. That is the parent's file being
 * wrong, so it is a 400 in plain words; anything else (ffmpeg missing) is ours
 * and stays a 500.
 *
 * @returns {Promise<{buffer: Buffer, durationMs: number}>} the bytes to store
 *   and how long they play — both describing the converted file, never the
 *   upload.
 */
const toDeviceAudio = async (file) => {
  try {
    return await toDeviceMp3(file.buffer);
  } catch (error) {
    if (error.transcodeFailed) {
      logger.warn(`[CUSTOM-CARD] Audio conversion failed: ${error.message}`);
      throw new ApiError('That recording could not be converted. Please try another file.', 400);
    }
    throw error;
  }
};
...
/**
 * An item's running time in seconds, or null when it was stored before
 * conversion started recording one. Milliseconds are what the column holds;
 * seconds are what the app renders, to one decimal.
 */
const durationSecondsOf = (item) => (
  item?.audio_duration_ms ? Math.round(Number(item.audio_duration_ms) / 100) / 10 : null
);
  ...
  return {
    ...
    contentPack: pack
      ? {
        ...
        // Measured off the stored MP3, not the upload. Null for a recording
        // stored before conversion existed — the app renders those without a
        // running time rather than guessing one.
        durationSeconds: durationSecondsOf(first),
        ...
        items: items.map((item) => ({
          ...
          durationSeconds: durationSecondsOf(item),
...
const toItemPayload = (item) => ({
  ...
  // Always present, never undefined — same rule as imageUrl below, and the same
  // failure if it is left out: a renumber would graft the running time of
  // whichever item used to hold this number onto this one.
  audioDurationMs: item.audio_duration_ms ? Number(item.audio_duration_ms) : null,
...
const addCustomCardContent = async (userId, kidId, files, { title, images = [] } = {}) => {
  ...
  // Convert before any upload, for the same all-or-nothing reason as validation:
  // ffmpeg failing on the third file must not leave two in the bucket.
  for (const entry of validated) {
    entry.audio = await toDeviceAudio(entry.file);
    if (entry.image) entry.imageBin = await toDeviceImage(entry.image, entry.imageInfo);
  }

  const appended = [];
  for (const [index, entry] of validated.entries()) {
    // The converted bytes, under a name that ends in the converted extension:
    // uploadCustomCardAudio reads the filename only to pick the S3 key's suffix,
    // and a WAV upload stored under a `.wav` key would hand the toy a file whose
    // name lies about what is inside it. The parent's own filename survives as
    // the item title below.
...
const replaceCustomCardItem = async (userId, kidId, itemNumber, file, { title, image } = {}) => {
  ...
  const audio = await toDeviceAudio(file);
...
const patchCustomCardItem = async (userId, kidId, itemNumber, {
  ...
  // Converted after the version check and before storage: it is the last step
