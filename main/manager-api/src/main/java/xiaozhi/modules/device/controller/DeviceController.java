...
public class DeviceController {
    ...
    @GetMapping("/{macAddress}/device-control-mode")
    ...
    public Result<String> getDeviceControlMode(@PathVariable("macAddress") String macAddress) {
        try {
            ...
            log.info("📋 [GET-DEVICE-MODE] Device control mode query requested for device MAC: {}", cleanMac);
            ...
            log.info("✅ [GET-DEVICE-MODE] Device {} device_mode: {}", cleanMac, deviceMode);

            return new Result<String>().ok(deviceMode);

        } catch (Exception e) {
            log.error("❌ [GET-DEVICE-MODE] Error getting device_mode for MAC {}: {}", macAddress, e.getMessage());
            return new Result<String>().error(e.getMessage());
        }
    }
}